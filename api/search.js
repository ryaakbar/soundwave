...
export default async function handler(req, res) {
    ...
    try {
        ...
        // Danzy API bisa return berbagai struktur
        ...
        // Normalize semua field yang mungkin beda nama
        const results = raw.map(item => ({
            ...
            duration:  item.duration || item.duration_ms
                ? fmtDuration(item.duration || item.duration_ms)
                : ""
        ...
        if (error.code === "ECONNABORTED") {
            return res.status(504).json({ error: "API timeout. Coba lagi." });
        }
...
// Convert ms or "3:54" string to "3:54" format
